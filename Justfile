...
##     and the per-axis recipes (`test-arc`, `test-asan`, etc.) — those are
...
format-nim:
    @if command -v nimpretty >/dev/null 2>&1; then \
      nimpretty src/nim_pty.nim src/nim_pty/*.nim tests/*.nim; \
    else \
...
bump-version version:
    sed -i 's/^version[[:space:]]*=.*/version       = "{{version}}"/' nim_pty.nimble
...
# "live release" build users will see — sanitizers catch different bugs in
# each.
#
# NB: on Nix-managed systems Nim's `--cc:clang` doesn't always resolve to
# the actual clang binary because the Nim wrapper substitutes `$CC` /
# `$CXX` for both `clang.exe` and `gcc.exe`. We therefore export
# `CC=clang` / `CXX=clang++` for these recipes so Nim ends up calling the
# right compiler. Inside `nix develop` clang is on PATH; outside, the
# `clang` package in flake.nix's devShell ensures it.
test-asan:
    @mkdir -p test-logs
...
#
# `--child-silent-after-fork=yes` is required because nim-pty's tests fork
# (and then exec) child processes; without it valgrind reports spurious
# "leaks" from the brief window between fork and exec where the child
# inherits the parent's heap. We also use `${PIPESTATUS[0]}` so the `tee`
# pipe doesn't mask valgrind's `--error-exitcode=1` — otherwise the recipe
# would always exit 0 even when valgrind detected real errors.
test-valgrind:
    #!/usr/bin/env bash
    set -euo pipefail
    mkdir -p test-logs
...
# Kept here so `just bench` resolves on every Metacraft repo.
bench:
    @echo "nim-pty has no benchmarks yet — see L2/L3 milestones for perf work."

# Run the example program in README.md to make sure the docs stay accurate.
test-readme:
    @mkdir -p test-logs
